...
internal class AudioStreamBuilder : LanguageMediaStreamBuilder<AudioStream>
{
    ...
    public override AudioStream Build()
    {
        ...
        {
            result.Codec = GetCodecIdByCodecName(AnalyzeUndefinedCodeс());
        }

        switch (result.Codec)
        {
            case AudioCodec.MpegLayer3:
                result.Codec = AnalyzeMpegLayer3();
                break;
            ...
            case AudioCodec.Truehd:
                (baseIndex, result.Codec) = AnalyzeMlpFeatures(result.Codec);
                break;
        }
    ...
    private (int BaseIndex, AudioCodec Codec) AnalyzeMlpFeatures(AudioCodec sourceCodec)
    {
        ...
        return (0, sourceCodec);
    }

    private AudioCodec AnalyzeMpegLayer3()
    {
        var formatProfile = Get((int)NativeMethods.Audio.Audio_Format_Profile, InfoKind.Text)?.Trim();
        return formatProfile?.ToLower() switch
        {
            "layer 2" => AudioCodec.MpegLayer2,
            "layer 1" => AudioCodec.MpegLayer1,
            _ => AudioCodec.MpegLayer3,
        };
    }

    private string AnalyzeUndefinedCodeс()
    {
        ...
        if (codecValue.Equals("PCM", StringComparison.OrdinalIgnoreCase))
        {
            ...
            codecValue += string.IsNullOrEmpty(endianness) ? string.Empty : " " + endianness;
        }
        if (codecValue.Equals("WMA", StringComparison.OrdinalIgnoreCase))
        {
            ...
            codecValue += string.IsNullOrEmpty(profile) ? string.Empty : profile;
        }
    ...
    private static string? ExtractInfo(string? source, int index) =>
        string.IsNullOrEmpty(source) ?
            default :
            InternalExtractInfo(source!, index);

    private static string? InternalExtractInfo(string source, int index) =>
    ...
    private static AudioCodec GetCodecIdByCodecName(string source) =>
        Codecs.TryGetValue(source, out var result) ?
            result :
            AudioCodec.Undefined;

    private static AudioCodec GetMlpCodecIdByAdditionalFeatures(string source) =>
        MlpCodecsAdditionalFeatures.TryGetValue(source, out var result) ?
            result :
            AudioCodec.Undefined;
}