...
internal class GeneralTagBuilder<T> where T : BaseTags, new()
{
    ...
    public virtual T Build()
    {
        ...
        foreach (var (tag, parseFunc) in GeneralTagItems)
        {
            ...
            if (!string.IsNullOrEmpty(value) && parseFunc(value, out var tagValue) && tagValue is not null)
            {
            ...
            cover != null)
        {
            ...
            if (itemCount > 0)
            {
                ...
                {
                    string data = coverDataItems.TryGet(i, string.Empty);
#if NET5_0_OR_GREATER
                    covers.Add(new CoverInfo(
                        ToBool(coverItems.TryGet(i, string.Empty)),
                        coverDescriptionItems.TryGet(i, string.Empty),
                        coverTypeItems.TryGet(i, string.Empty),
                        coverMimeItems.TryGet(i, string.Empty),
                        string.IsNullOrEmpty(data) ? null : Convert.FromBase64String(data)));
#else
                    covers.Add(new CoverInfo
                    {
                        Exists = ToBool(coverItems.TryGet(i, string.Empty)),
                        Description = coverDescriptionItems.TryGet(i, string.Empty),
                        Type = coverTypeItems.TryGet(i, string.Empty),
                        Mime = coverMimeItems.TryGet(i, string.Empty),
        ...
        {
            var value = MediaInfo.Get(StreamKind.Audio, StreamPosition, "DATE_TAGGED")?.TryGetDate(out DateTime res) ?? false ?
                (DateTime?)res :
                null;
            if (value != null)
            {
        ...
        {
            var value = MediaInfo.Get(StreamKind.Audio, StreamPosition, "RATING")?.TryGetDouble(out double res) ?? false ?
                (double?)res :
                null;
            if (value != null)
            {
        ...
        {
            var value = MediaInfo.Get(StreamKind.Audio, StreamPosition, "Released_Date")?.TryGetDate(out DateTime res) ?? false ?
                (DateTime?)res :
                null;
            if (value != null)
            {
        ...
        if (!result.GeneralTags.ContainsKey((NativeMethods.General)1000) &&
            (MediaInfo.Get(StreamKind.General, StreamPosition, "Stereoscopic")?.TryGetBool(out var stereoscopic) ?? false))
        {
        ...
        if (!result.GeneralTags.ContainsKey((NativeMethods.General)1001) &&
           (MediaInfo.Get(StreamKind.General, StreamPosition, "StereoscopicLayout")?.TryGetStereoMode(out var stereoscopicLayout) ?? false))
        {
        ...
        if (!result.GeneralTags.ContainsKey((NativeMethods.General)1002) &&
           (MediaInfo.Get(StreamKind.General, StreamPosition, "StereoscopicSkip")?.TryGetInt(out int stereoscopicSkip) ?? false))
        {
