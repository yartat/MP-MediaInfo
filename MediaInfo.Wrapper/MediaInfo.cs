...
public class MediaInfo : IDisposable
{
    ...
    public IntPtr OpenBufferInit(long fileSize, long fileOffset) =>
        Handle == IntPtr.Zero ?
            IntPtr.Zero :
            NativeMethods.MediaInfo_Open_Buffer_Init(Handle, fileSize, fileOffset);
    ...
    public IntPtr OpenBufferContinue(IntPtr buffer, IntPtr bufferSize) =>
        Handle == IntPtr.Zero ?
            IntPtr.Zero :
            NativeMethods.MediaInfo_Open_Buffer_Continue(Handle, buffer, bufferSize);
    ...
    public unsafe int OpenBufferContinue(byte* buffer, int bufferSize) =>
        Handle == IntPtr.Zero ?
            0 :
            (int)NativeMethods.MediaInfo_Open_Buffer_Continue(Handle, buffer, (IntPtr)bufferSize);
    ...
    public long OpenBufferContinueGoToGet() =>
        Handle == IntPtr.Zero ?
            0 :
            NativeMethods.MediaInfo_Open_Buffer_Continue_GoTo_Get(Handle);
    ...
    public IntPtr OpenBufferFinalize() =>
        Handle == IntPtr.Zero ?
            IntPtr.Zero :
            NativeMethods.MediaInfo_Open_Buffer_Finalize(Handle);
    ...
    public IntPtr StateGet() =>
        Handle == IntPtr.Zero ?
            IntPtr.Zero :
            NativeMethods.MediaInfo_State_Get(Handle);
    ...
    public int CountGet(StreamKind streamKind, int streamNumber) =>
        Handle == IntPtr.Zero ?
            0 :
            (int)NativeMethods.MediaInfo_Count_Get(Handle, (IntPtr)streamKind, (IntPtr)streamNumber);
